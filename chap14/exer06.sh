...
# Take the first match of $1 against $PWD and replace with $2 using sed.  If
# such a match exists, then try to cd to that directory, otherwise return 1.
...
korn_subst () {
    ...
    if [ "$new_path" = "$PWD" ]
    then
	echo "error: string $old not matched against $PWD" >&2
	return 1
    fi
...
#                         grep
#     cdh old new     <-- cd to the path obtained by replacing the first match 
#                         of old in PWD with new, using sed
...
cdh ()  {
    ...
    then
	set -- $HOME
    elif [ $# -eq 2 ]                   # emulate Korn partial substitution
    then
	korn_subst $@
	return
    fi
